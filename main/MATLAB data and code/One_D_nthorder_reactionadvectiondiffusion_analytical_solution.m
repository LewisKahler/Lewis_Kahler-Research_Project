...
for j = 1:nt
    for i = 1:nx
        U(i,j) = 1*0.5*(erfc(x(i)-v*t(j)/(2*(D*t(j))^(1/2)))+exp(v*(x(i))/D)*erfc(x(i)-v*t(j)/(2*(D*t(j))^(1/2))));
...
for j = 1:nt -1
    for i = 2:nx -1
        UN(i,j+1) = UN(i,j) + FD*(UN(i-1,j)-2*UN(i,j)+UN(i+1,j)) - 0.5*Fv*(UN(i+1,j)-UN(i-1,j)) - k*dt*(UN(i,j))^n;
    end
end
